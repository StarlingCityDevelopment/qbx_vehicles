...
---@field group? string group of the vehicle
---@field garage? string
...
local function createPlayerVehicle(request)
    ...
    if not triggerEventHooks('createPlayerVehicle', { citizenid = request.citizenid, group = request.group, garage = request.garage, props = props }) then
        return nil, {
            code = 'hook_cancelled',
...
local function setPlayerVehicleOwner(vehicleId, citizenid, group)
    assert(vehicleId ~= nil, "required field vehicleId was nil")
    if not triggerEventHooks('changeVehicleOwner', { vehicleId = vehicleId, newCitizenId = citizenid, newGroup = group }) then
        return false, {
            code = 'hook_cancelled',
...
---@param idType 'citizenid'|'group'|'license'|'plate'|'vehicleId'
